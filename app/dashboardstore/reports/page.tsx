...
export default async function StoreReportsPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  if (!canUseUserPermission(user, PermissionKey.VIEW_STORE_REPORTS)) redirect("/dashboardcm");
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        ...
        <AdminSiteScopeSelector action="/dashboardstore/reports" scope={scope} title="Site สำหรับรายงาน Store" />

        <section className="rounded-3xl border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[var(--shadow)]">
          <form action="/dashboardstore/reports" className="grid gap-3 md:grid-cols-[1fr_1fr_auto] md:items-end">
