...
async function receiveStockAction(formData: FormData) {
  ...
  redirect(
    `/dashboardstore/receive?organizationId=${encodeURIComponent(scope.organization.id)}&plantId=${encodeURIComponent(scope.plant.id)}&saved=1`,
  );
}

export default async function ReceivePage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  const user = await requireUser();
  if (!canUseUserPermission(user, PermissionKey.RECEIVE_STOCK)) redirect("/dashboardcm");
