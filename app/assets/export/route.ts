...
export async function GET(request: Request) {
  ...
  const assets=await db.asset.findMany({where:{plantId:scope.plant.id,registrationStatus:"ACTIVE"},include:{system:true,family:true,assetClass:true,assetType:true,zone:true,parent:true},orderBy:{code:"asc"}});
  const rows=assets.map(a=>({"Parent Code":a.parent?.code||"","System":a.system?.code||"","Asset Level":a.assetLevel,"Discipline":a.discipline||"","Tag / KKS":a.tagKks||"","Registration Code":a.registrationCode||"","Key Specification":a.keySpecification||"","Metadata JSON":a.metadataJson||"","Migration Status":a.migrationStatus,"Family Code":a.family?.code||"","Component Code":a.componentCode||"","Asset Class":a.assetClass?.nameTh||"","Asset Type":a.assetType?.code||"","Asset Code":a.code||"","ชื่อภาษาไทย":a.nameTh,"English Name":a.nameEn||"","Area / Zone":a.zone?.name||"","Installation Location":a.installationLocation||"","Manufacturer":a.manufacturer||"","Model":a.model||"","Serial Number":a.serialNumber||"","Operating Status":a.operatingStatus,"Criticality":a.criticality}));
