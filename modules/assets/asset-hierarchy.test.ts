...
describe("Asset hierarchy contract", () => {
 it.each([main, sub, part, { ...part, parentId: "main" }, { ...part, parentId: null }])("accepts valid edge for $id/$parentId", node => expect(validateAssetHierarchy(node, [main, sub, part])).toEqual([]));
 it.each([
 ...
 it("review status does not bypass ordinary validation", () => expect(validateAssetHierarchy({ ...sub, parentId: null, migrationStatus: "NEED_PARENT_REVIEW" }, [])).toContain("PARENT_REQUIRED"));
