...
describe("PM calendar views", () => {
  it("renders a 42-cell accessible desktop grid with compact plan summaries", () => {
    render(<PmCalendar canManage month="2026-08-01" plans={[{ id: "p", plannedDateKey: "2026-08-15", status: "DRAFT", number: null, draftGroups: [{ pmGroup: { id: "g", code: "G", name: "Group" } }], _count: { works: 0 } }]} scopeQuery="organizationId=o&plantId=s" today="2026-08-20" />);
    expect(screen.getByRole("grid")).toBeTruthy(); expect(screen.getAllByRole("row")).toHaveLength(7); expect(screen.getAllByRole("columnheader")).toHaveLength(7); expect(screen.getAllByRole("gridcell")).toHaveLength(42); expect(screen.getByText("1 PM Groups")).toBeTruthy(); expect(screen.getByRole("link", { name: "Today" }).getAttribute("href")).toContain("date=2026-08-20"); expect(screen.queryByText("Pump asset one")).toBeNull();
  });
  ...
  it("offers explicit month and day views with the active state", () => {
    ...
    expect(screen.getByRole("link", { name: "Day" }).getAttribute("aria-current")).toBe("page");
